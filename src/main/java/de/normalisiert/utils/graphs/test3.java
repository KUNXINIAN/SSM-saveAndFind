...
public class test3{
     ...
     public static void DFS(HashMap gra, ArrayList res, boolean[] visited, ArrayList path, int node, int start, HashMap lut){
         ...
         //三层查找表
         if(path.size() == 6){
             HashMap map = (HashMap) lut.get(node);
             if(map.containsKey(start)){
                 HashSet<Integer> set = (HashSet) map.get(start);
                 for(Integer integer : set){
                     if(integer == -1||integer.equals(-1)) { //六环情况
                         res.add(new ArrayList<>(path));
                         continue;
                     }
...
//             if(node == temp) continue;
         ...
         public int compare(ArrayList o1, ArrayList o2) {
             if(o1.size() == o2.size()){
                 for (int i = 0; i < o1.size(); i++){
                     if(o1.get(i) == o2.get(i)||o1.get(i).equals(o2.get(i))){
                         continue;
                     }
    ...
    public static void main(String[] args) throws IOException {
        ...
        //数据读入
//        String fileName = "src/com/huawei/data/test_data.txt";
        ...
        for (Integer key : gra.keySet()) {
            ArrayList arr3F = gra.get(key);
            for (Object obj : arr3F) {
                ArrayList arr2F = gra.get(obj);
                if(arr2F != null && !arr2F.isEmpty()) {

                    for (Object keyfor2F : arr2F) {
                        if(!lut.containsKey(key)) { //lut无key
                            ...
                            if(!lutSecondFloor.containsKey(keyfor2F)){ //lutSecondFloor无key
                                ...
                                lutThirdFloor.add((Integer) obj);
                                lutSecondFloor.put((Integer) keyfor2F, lutThirdFloor);
                            }
                            if(!lutSecondFloor.containsKey(obj)){ //代替环中6->1，只有6层无7层时
                                ...
                                lutSecondFloor.put((Integer) obj, nullThirdFloor);
                            }
                            lut.put(key, lutSecondFloor);
                        }else { //lut有key
                            HashMap lutSecondFloor = lut.get(key);
                            ...
                            if(lutSecondFloor.containsKey(keyfor2F) && lutSecondFloor.containsKey(obj)) {
                                HashSet lutThirdFloor = (HashSet) lutSecondFloor.get(keyfor2F);
        ...
        //输出
        String outputFileName = "/Users/likunxin/Downloads/huaweiTest/output_data2.txt";
